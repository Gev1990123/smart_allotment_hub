...
@app.post("/api/plant-profiles")
async def create_plant_profile(
    body: PlantProfileCreate,
    current_user: Dict = Depends(get_current_user)
):
    """Create a new plant profile. Any authenticated user can create; admin not required."""
...
@app.put("/api/plant-profiles/{profile_id}")
async def update_plant_profile(
    ...
    try:
        ...
        # Name uniqueness check (allow keeping the same name)
        cur.execute(
...
@app.delete("/api/plant-profiles/{profile_id}")
async def delete_plant_profile(
    ...
    - Any sensor_plant_assignments pointing here are removed by ON DELETE CASCADE.
    """
    conn = get_connection()
    cur = conn.cursor()
    try:
...
@app.put("/api/sensors/{sensor_id}/plant-profile")
async def assign_plant_profile(
    ...
    cur = conn.cursor()
    try:
        # Verify sensor exists and user can access it
        cur.execute("""
...
@app.delete("/api/sensors/{sensor_id}/plant-profile")
async def remove_plant_profile(
    sensor_id: int,
    current_user: Dict = Depends(get_current_user)
):
...
@app.get("/api/sensors/{sensor_id}/moisture-status")
async def sensor_moisture_status(
    sensor_id: int,
    current_user: Dict = Depends(get_current_user)
):
    """
...
@app.get("/api/sensors/{sensor_id}/moisture-events")
async def sensor_moisture_events(
    ...
    try:
        ...
        rows = cur.fetchall()
        return {"events": [
            {"reading": float(r[0]), "expected_min": float(r[1]),
             "expected_max": float(r[2]), "status": r[3],
...
# ---------------------------------------------------------
# PLANT PROFILES PAGE (HTML template)
# ---------------------------------------------------------

@app.get("/plant-profiles")
